...
class AnnotationFormConfig {
  ...
  const AnnotationFormConfig({
    ...
    this.scoreMaxTotal = '50',
    this.scoreMaxCritere = '10',
    this.noteMin = 0,
    this.noteMax = 5,
...
class AnnotationNoteManager {
  ...
  void _initDefaultNotes() {
    for (final config in configuration) {
      _notes['${config.critereId}_${config.element1Id}'] = 0.0;
      _notes['${config.critereId}_${config.element2Id}'] = 0.0;
    }
  }

  double getNote(String critereId, String elementId) {
    return _notes['${critereId}_$elementId'] ?? 0.0;
  }
  ...
  double getCritereTotal(ConfigurationElementEvaluation config) {
    return getNote(config.critereId, config.element1Id) +
  ...
  bool get allElementsRated {
    for (final config in configuration) {
      if (getNote(config.critereId, config.element1Id) == 0) return false;
      if (getNote(config.critereId, config.element2Id) == 0) return false;
    }
    return true;
  }
  ...
  List<ScoreAnnotation> toScoreAnnotations() {
    return configuration.map((config) {
      return ScoreAnnotation(
        ...
        noteElement1: getNote(config.critereId, config.element1Id),
        element2Id: config.element2Id,
        noteElement2: getNote(config.critereId, config.element2Id),
      );
    }).toList();
  }
